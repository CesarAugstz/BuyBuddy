...
func Setup(e *echo.Echo, cfg *config.Config, db *gorm.DB) *organizer.Service {
	...
	knowledgeAssistantHandler := handlers.NewKnowledgeAssistantHandler(cfg, chatRepo, prefsRepo, knowledgeRepo, knowledgeOrganizer)
	...
	knowledgeAssistant := knowledge.Group("/assistant")
	knowledgeAssistant.Use(echomiddleware.BodyLimit("32K"))
	knowledgeAssistant.POST("/ask", knowledgeAssistantHandler.AskQuestion)
	knowledgeAssistant.GET("/conversation/:conversationId", knowledgeAssistantHandler.GetConversationHistory)
